...
El `compilador` es necesario para poder ejecutar la compilación de los archivos `.pwn` mientras que los `extras` serán archivos y librerías que necesitarás para poder encender el servidor, como `samp-server.exe` y archivos `.dll` o `.so` en caso de linux, es necesario descargar el archivo para la plataforma correspondiente, si ejecutarás el servidor en linux descarga el que diga `...-linux` o `...-windows` si es el caso. También tenemos disponible `base-de-datos` que trata de un archivo `.sql` que contiene la base de datos del servidor.

- [`compilador-windows`](https://github.com/daniscript18/clan-wars/releases)
- [`compilador-linux`](https://github.com/daniscript18/clan-wars/releases)
- [`extras-windows`](https://github.com/daniscript18/clan-wars/releases)
